<script lang="ts">
	...
	import { formatPrice, getOrderSummary } from '../../scripts/helpers';
	import type { OrderType } from '../../../types/order.type';

	export let catering_id: string;
	let orders: OrderType[] = [];
	let loading = true;
	let orderSummary = {} as Record<string, number>;

	onMount(async () => {
		orders = (await getCateringOrders(catering_id)).data;
		loading = false;
		orderSummary = getOrderSummary(orders);
	});
...
{:else}
	<div class="py-4">
		...
		{#each Object.keys(orderSummary) as key}
			<div class="flex flex-row gap-2">
				<div>{key}: </div>
				<div>{orderSummary[key].toString()}</div>
			</div>
		{/each}
	</div>
