...
<div id="navbar-container" class="font-inter fixed w-full backdrop-blur-sm">
	<div class="py-2 container mx-auto  grid grid-cols-2 md:grid-cols-3 w-full h-24">
		...
		<div class="hidden md:flex flex-row justify-center place-items-center font-karla">
			<div class="flex flex-row h-fit relative place-items-center bg-gray-300 rounded-full text-lg">
				{#each menus as menu}
  ...
  <path fill-rule="evenodd"
		...
		<div class="flex flex-row justify-end place-items-center gap-3">
			<div class="flex flex-col place-items-end">
				...
				{/if}
				<div class="flex-row gap-1 text-sm">
			...
			{#if data.user}
				<form method="POST" action="/logout">
					<button class="w-fit rounded-xl bg-orange-sig text-white rotate-180 h-full px-4 py-2.5">
	...
	<div class="bg-gradient-to-b from-slate-100 to-gray-100 font-inter mt-8 text-black text-xs">
		<div class="py-2 responsive-container w-full">
			<div class="my-2 flex flex-row justify-between w-full">
				...
				<div class="text-xs font-medium flex flex-col place-items-end gap-3">
					<img src="{slcatering_logo}" alt="SLCatering Logo" class="h-12" />
					<div>
						Developed by NJ23-1 ML23-1 IC23-1
					</div>
				</div>
			</div>
		</div>
